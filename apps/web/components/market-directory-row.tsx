...
export function MarketDirectoryRow({ asset }: { asset: InvestmentAsset }) {
  ...
  return <Link prefetch={false} href={href} className="market-row">
    <span className="market-company"><AssetLogo imageUrl={asset.imageUrl} symbol={asset.symbol} /><span><strong>{asset.name}</strong><small>{asset.symbol} · {asset.provider === "prestocks" ? "PreStocks" : "xStocks"}</small></span></span>
