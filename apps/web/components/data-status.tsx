export function DataStatus({ fetchedAt, stale }: { fetchedAt: string; stale: boolean }) {
  ...
  return (
    <div className="text-xs leading-6 text-muted">
      ...
      {stale && <p role="status" className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-900">PreStocks is temporarily unavailable. Showing recently saved data; prices may have changed.</p>}
    </div>
  );
}
