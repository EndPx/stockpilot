...
export function AssetLogo({ imageUrl, symbol, large = false }: { imageUrl: string | null; symbol: string; large?: boolean }) {
  ...
  return (
    <span className={`inline-flex shrink-0 items-center justify-center overflow-hidden rounded-xl border border-line bg-white font-semibold text-muted ${large ? "h-16 w-16 text-xl" : "h-11 w-11 text-sm"}`}>
      {imageUrl && !failed ? (
