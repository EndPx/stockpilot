...
## Discovery exception found during live QA — owner decision pending
...
The owner was asked whether listed private-market funds should remain excluded or be explicitly exempted as public instruments. **No exception is assumed.** Until answered, this phase must not claim that all 1,026 products were admitted or that an exhaustive private-exposure audit passed. The live SpaceX xStock SPCXx is also an official issuer product ([source](https://assets.backed.fi/products/spacex-xstock)); it must not be confused by ticker/name with the distinct PreStocks SPACEX asset or treated as a user-created synthetic mint.
