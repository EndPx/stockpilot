...
connect a Solana wallet, prove ownership of that wallet, and review its read-only
StockPilot portfolio. The live company table carries Markets; the overview makes
funding and official PreStocks holdings legible without implying trade execution.
...
and transaction controls. Show a compact set of required portfolio totals, the
provider, readable prices, an obvious path to asset details, and an equally clear
connection status. A keyboard-only user can operate every action; a mobile user
receives the same states without clipped controls.
